...
// Genera: Facturas Recibidas (tickets) + Listado de Proveedores
...
$seccion = $_GET['seccion'] ?? 'ambas'; // 'facturas' | 'proveedores' | 'ambas'
$desde   = $_GET['desde']   ?? date('Y-01-01');
$hasta   = $_GET['hasta']   ?? date('Y-12-31');

// --- Facturas recibidas (tickets de compra) ---
// Si existe la tabla tickets_compra la usamos; si no, tiramos del libro diario cuentas 6xx
$tabla_tickets = false;
try {
...
if ($tabla_tickets) {
    ...
    // Fallback: gastos del libro diario (cuentas grupo 6)
    $stmt_t = $pdo->prepare(
        "SELECT ld.fecha,
                ld.concepto,
                'Sin proveedor' AS proveedor,
                '—' AS nif_cif,
                ld.debe AS base_imponible,
                0 AS cuota_iva,
                ld.debe AS total
...
<table>
    ...
    <?php if (empty($proveedores)): ?>
        <tr><td colspan="7" style="text-align:center;padding:16px;color:#aaa">No hay proveedores registrados.</td></tr>
